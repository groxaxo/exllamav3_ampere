#!/usr/bin/env bash
# Quantize wangzhang/Qwen3.5-122B-A10B-abliterated to EXL3 5.0 bpw
set -euo pipefail
...
WORK_DIR="${EXL3_DIR}/models/work_122b_ablit_5bpw"
OUT_DIR="${EXL3_DIR}/models/Qwen3.5-122B-A10B-abliterated-exl3-5bpw"
LOG="${EXL3_DIR}/logs/quantize_122b_ablit_5bpw.log"
...
{
  printf '[%s] Starting EXL3 5.0 bpw quantization\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf 'Source:  %s\n' "${LOCAL_SRC}"
  printf 'Work:    %s\n' "${WORK_DIR}"
  printf 'Output:  %s\n' "${OUT_DIR}"
  printf 'Log:     %s\n' "${LOG}"
