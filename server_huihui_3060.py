...
MODEL_DIR = "/home/op/models/Huihui-Qwen3.5-9B-abliterated-exl3-6.00bpw"
# RTX 3060s are GPU indices 2 and 3
GPU_DEVICES = [2, 3]
PORT = int(os.getenv("PORT", "8002"))

app = FastAPI(title="ExLlamaV3 Huihui-Qwen3.5-9B Server (3060 x2)")
...
async def load_model():
    ...
    print(f"Loading model on GPUs {GPU_DEVICES} (RTX 3060 x2)...")
    # Set primary device to first 3060
    torch.cuda.set_device(GPU_DEVICES[0])

    config = Config.from_directory(MODEL_DIR)
    model = Model.from_config(config)

    # Split across both 3060s — ~12 GB VRAM each, 9B at 6bpw ~6.75 GB total fits easily on one,
    # but split for cache headroom: 10 GB + 10 GB budget leaves plenty for large context
    model.load(
        use_per_device={GPU_DEVICES[0]: 10.0, GPU_DEVICES[1]: 10.0},
        progressbar=True,
    )
    ...
    print("Model loaded! Ready on port", PORT)
...
async def list_models():
    return {
        ...
        "data": [
            {
                "id": "Huihui-Qwen3.5-9B-abliterated-exl3-6.00bpw",
                "object": "model",
                "created": int(time.time()),
                "owned_by": "local",
            }
