...
class Qwen3VLConfig(Config):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            directory, {"text": Qwen3VLModel, "vision": Qwen3VLVisionModel}, **kwargs
        )
        ...
        self.num_q_heads = self.read_cfg(
            int, "text_config->num_attention_heads", no_default
        )
        self.num_kv_heads = self.read_cfg(
            int, "text_config->num_key_value_heads", self.num_q_heads
        )
        ...
        self.intermediate_size = self.read_cfg(
            int, "text_config->intermediate_size", no_default
        )

        # Norms
        self.rms_norm_eps = self.read_cfg(
            float, "text_config->rms_norm_eps", no_default
        )

        # Layers
        self.num_hidden_layers = self.read_cfg(
            int, "text_config->num_hidden_layers", no_default
        )
...
def read_qwen3_vl_vision_config(config_dict: dict):
    ...
    model_types = ["qwen3_vl", "qwen3_vl_moe", "qwen3_5", "qwen3_5_moe"]
...
def read_qwen3_vl_pp_config(config_dict: dict):
    if "image_processor" in config_dict and isinstance(
        config_dict["image_processor"], dict
    ):
        config_dict = config_dict["image_processor"]
...
class Qwen3VLVisionModel(Model):
    ...
    def preprocess(self, images: Image | list[Image]) -> (torch.Tensor, tuple):
        v = self.config.vision
    ...
    def get_image_embeddings(
        ...
        inv_freq = position_embedding_grid_2d(
            grid_thw, v.head_dim, v.spatial_merge_size, v.rope_theta
        )
        params = {
            "causal": False,
        ...
        for i in range(embedding_tensor.shape[0]):
            ...
            token_string = torch.tensor(
                [[id_start] + [-1] * num_emb_tokens + [id_end]], dtype=torch.long
            )
            ...
            mme = MMEmbedding(
                ...
                deepstack_embeddings=[de.squeeze(0) for de in params["deepstack"]]
                if "deepstack" in params
                else None,
                grid_thw=grid_thw,
